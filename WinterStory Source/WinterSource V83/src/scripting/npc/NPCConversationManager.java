...
public class NPCConversationManager extends AbstractPlayerInteraction {
	...
	public void sendDimensionalMirror(String text) {
        ...
        public boolean startFishing(int baitLevel) {
			if(!GameConstants.isFishingArea(c.getPlayer().getMapId()))
			{
				c.getPlayer().yellowMessage("You are not in the fishing area!");
				return false;
			}
			else if(!ItemConstants.isFishingChair(c.getPlayer().getChair()))
			{
				c.getPlayer().yellowMessage("You are not sitting in the fishing chair!");
				return false;
			}
			else if(baitLevel < 0 || !(c.getPlayer().getPosition().getY() > 0) || !YamlConfig.config.server.USE_FISHING_SYSTEM)
			{
				c.getPlayer().yellowMessage("Error happend or fishing is disabled!");
				return false;
			}
			else
			{
				c.getPlayer().sitChair(3011000); // Automatic chair
				c.getPlayer().getWorldServer().registerFisherPlayer(c.getPlayer(), baitLevel);
				c.getPlayer().yellowMessage("You just started Fishing!");
				return true;
			}
	...
	public String getItemAsName(int itemId) {
		return MapleItemInformationProvider.getInstance().getName(itemId);
	}

	public void useVotePoints(int points)
	{
		c.useVotePoints(points);
	}

	public int getVotePoints()
	{
		return c.getVotePoints();
	}

	public void addVotePoints(int points)
	{
		c.addVotePoints(points);
	}

	public void setDonorPoints(int points) {
		c.getPlayer().setDonorPoints(points);
	}

	public int getDonorPoints() {
		return c.getPlayer().getDonorPoints();
	}

	public void giveDonorPoints(int points) {
		c.getPlayer().setDonorPoints(c.getPlayer().getDonorPoints() + points);
	}
	...
	public void doGachapon() {
		...
		getPlayer().addNGachM(1);

		getPlayer().addNGachW(1);
		...
		if (item.getTier() > 0){ //Uncommon and Rare
			...
			getPlayer().addRGachW(1);
			getPlayer().addRGachM(1);
		}
	}
