 ...
 *
 * @author XoticStory
 * @author DrScript - concurrency protection
 */
public class MapleHiredMerchant extends AbstractMapleMapObject {
            ...
            if (c.getPlayer().getMeso() >= price) {
                if (canBuy(c, newItem)) {
                    ...
                    synchronized (sold) {
                        ...
                        c.getPlayer().AddBoughtItemW(price);
                        c.getPlayer().AddBoughtItemM(price);
                    }
    ...
    private void announceItemSold(Item item, int mesos, int inStore) {
        ...
        if(player != null && player.isLoggedinWorld()) {
            ...
            player.AddSellItemW(mesos);
            player.addSellItemM(mesos);
        }
    }
