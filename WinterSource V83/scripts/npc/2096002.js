...
var pulsar = 4001126, items3543 = [50,100];
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selection == 0 || selection == 1) {
            for (var i = 0; i < (selection == 0 ? maple35.length : maple43.length); i++) {
                text += "\r\n#L"+i+"##i"+(selection == 0 ? maple35 : maple43)[i]+"# #t"+(
                    selection == 0 ? maple35[i] : maple43[i])+"#- "+items3543[sel]+" Maple Leafs.";
            }
            cm.sendSimple(text);
        } else {
        ...
        sel2 = selection;
        if (sel == 0 || sel == 1) {
            if (!cm.haveItem(pulsar, items3543[sel])) {
                cm.sendOk("You do not have enough Maple Leafs. for this transaction. You are currently lacking"+
                    " "+(items3543[sel] - cm.itemQuantity(pulsar))+" Maple Leafs.");
                cm.dispose();
            } else {
                cm.sendOk("You have purchased a #i"+(sel == 0 ? maple35[selection] : maple43[selection])+"# for"+
                    " #b"+items3543[sel]+"#k Maple Leafs.");
                cm.gainItem((sel == 0 ? maple35[selection] : maple43[selection]), 1);
                cm.gainItem(pulsar, -items3543[sel]);
                cm.dispose();
            }
        } else {
         ...
         ? fag = 2 : (selection == 6 || selection == 7) ? fag = 3 : (selection == 8 || selection == 9) ? fag = 4 : selection == 10 ?
           fag = 5 : selection == 11 ? fag = 6 : selection == 12 ? fag = 7 : selection == 13 ? fag = 8 : selection == 14 ? fag = 9
            ...
            text = "Here are the following items you can use, in combination with Maple Leafs., to upgrade your item to the next"+
                " tier: #b#t"+maple64[selection]+"##k\r\n\r\n";
                for (var i = 0; i < req64[fag].length; i++) {
                    text += "#L"+i+"##i"+req64[fag][i]+"# - #b"+cost64[i]+" #t"+pulsar+"##k#l\r\n";
                }
            cm.sendSimple(text);
        }
    } else if (status == 2) {
        if (!cm.haveItem(pulsar, cost64[selection]) || !cm.haveItem(req64[fag][selection])) {
            cm.sendOk("You do not have the sufficient item(s) needed to complete this transaction. Please check the requirements again.");
            cm.dispose();
        } else {
            cm.sendOk("You have received a #i"+maple64[sel2]+"# in exchange for #b"+cost64[selection]+"#k #i"+pulsar+"#.");
            cm.gainItem(pulsar, -cost64[selection]);
